...
class DialogueCharacter extends FlxSprite
{
	...
	public function new(x:Float = 0, y:Float = 0, character:String = null)
	{
		...
		frames = Paths.getSparrowAtlas('menus/dialogue_editor/characters/' + jsonFile.image);
		reloadAnimations();
	...
	public function reloadCharacterJson(character:String) {
		var characterPath:String = 'images/menus/dialogue_editor/characters/' + character + '.json';
		var rawJson = null;
		...
		if(!FileSystem.exists(path)) {
			path = Paths.getSharedPath('images/menus/dialogue_editor/characters/' + DEFAULT_CHARACTER + '.json');
		}
