...
class WeekEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		var ui_tex = Paths.getSparrowAtlas('menus/story_menu/campaign_menu_UI_assets');
		...
		var tracksSprite:FlxSprite = new FlxSprite(FlxG.width * 0.07, bgSprite.y + 435).loadGraphic(Paths.image('menus/story_menu/Menu_Tracks'));
	...
	function reloadBG() {
		...
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(Paths.modsImages('backgrounds/menu_' + assetName)) || #end
			Assets.exists(Paths.getPath('menus/story_menu/backgrounds/menu_' + assetName + '.png', IMAGE), IMAGE)) {
	...
	function reloadWeekThing() {
		...
		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(Paths.modsImages('menus/story_menu/week_names/' + assetName)) || #end
			Assets.exists(Paths.getPath('images/menus/story_menu/week_names/' + assetName + '.png', IMAGE), IMAGE)) {
				weekThing.loadGraphic(Paths.image('menus/story_menu/week_names/' + assetName));
				isMissing = false;
			}
		}
		...
		if(isMissing) {
			...
			missingFileText.text = 'MISSING FILE: images/menus/story_menu/week_names/' + assetName + '.png';
		}
